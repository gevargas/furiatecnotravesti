...
La versión para Codespaces usa por defecto **Qwen2.5 0.5B Instruct Q4_K_M** mediante Ollama.
Es un modelo multilingüe cuantizado de aproximadamente **398 MB**, adecuado para un Codespace
estándar sin GPU. LatamGPT continúa disponible como opción remota para investigación y evaluación.

## Inicio rápido en Codespaces

1. Abre esta carpeta en un Codespace.
2. Espera a que la configuración instale Ollama y descargue el modelo pequeño.
3. Ejecuta:
...
El puerto `8501` se abrirá automáticamente. La inferencia ocurre dentro del Codespace y no
necesita claves de API.

Configuración conservadora: modelo Q4 de 398 MB, una sola generación y un solo modelo cargado,
dos hilos de CPU, contexto de 4096 tokens y respuestas de hasta 240 tokens.
...
|---|---:|---:|---|
| `ollama` | No | No | modelo pequeño local recomendado para Codespaces |
| `demo` | No | No | probar interfaz y pipeline sin inferencia |
...
## Paso 3 — Ejecutar con el modelo pequeño local
...
sin enviar ninguna conversación fuera del Codespace. Si necesitas comprobar solamente las
reglas sin inferencia, cambia temporalmente a `FURIA_PROVIDER=demo`.

### Instalación manual dentro de un Codespace existente
...
Para comprobar el servidor local:
